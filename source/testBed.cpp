...
#pragma config(Motor,  port1,           frontRightMotor, tmotorVex393_HBridge, PIDControl, driveRight, encoderPort, I2C_3)
#pragma config(Motor,  port2,           frontLeftMotor, tmotorVex393_MC29, PIDControl, reversed, driveLeft, encoderPort, I2C_4)
#pragma config(Motor,  port9,           backRightMotor, tmotorVex393_MC29, PIDControl, reversed, driveRight, encoderPort, I2C_2)
#pragma config(Motor,  port10,          backLeftMotor, tmotorVex393_HBridge, PIDControl, driveLeft, encoderPort, I2C_1)
...
task main()
{
	...
	//while(true)
	{
		...
		while(running)
		{
			...
			if(lSonar <= mSonar || rSonar > lSonar)//((lSonar > (mSonar - 2)) && (lSonar < (mSonar + 2)) && (rSonar > mSonar)) || (lSonar < (mSonar - 1)))//Move left
			{
				speedRight = -1 * (rSonar - lSonar);
			}
			if(rSonar <= mSonar || lSonar > rSonar)//((rSonar > (mSonar - 2)) && (rSonar < (mSonar + 2)) && (lSonar > mSonar)) || (rSonar < (mSonar - 1)))//Move right
			{
				speedRight = 1 * (lSonar - rSonar);
			}
			...
			for(int it = 3; it; it--)
			{
				if((mSonar < 11)/* && mSonar > 7) && (lSonar == rSonar)*/ && (lSonar > (mSonar + 30) && rSonar > (mSonar + 30)))
				{
					if(it == 1)
					{
						move(0, 40, 0);
