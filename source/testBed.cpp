...
task main()
{
	...
	while(running)
	{
		...
		//}
		if(((lSonar > (mSonar - 2)) && (lSonar < (mSonar + 2)) && (rSonar > mSonar)) || (lSonar < (mSonar - 1)))//Move left
		{
			speedRight = -55;
		}
		else if(((rSonar > (mSonar - 2)) && (rSonar < (mSonar + 2)) && (lSonar > mSonar)) || (rSonar < (mSonar - 1)))//Move right
		{
			speedRight = 55;
		}
		...
		for(int it = 3; it; it--)
		{
			if((mSonar < 11 && mSonar > 7)/* && (lSonar == rSonar)*/ && (lSonar > (mSonar + 40) && rSonar > (mSonar + 40)))
			{
				if(it == 1)
					running = false;
			}
